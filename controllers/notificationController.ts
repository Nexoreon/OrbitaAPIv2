...
import { toObjectId, sendError, sendDelayedNotification } from '../utils/common';
import Notification from '../models/notificationModel';

const message404 = 'Такого уведомления не найдено в системе!';
...
export const createNotification = catchAsync(async (req, res) => {
    ...
    if (sendOut !== Date.now()) sendDelayedNotification(req.body);
    res.status(201).json({
        status: 'ok',
...
export const getNotification = catchAsync(async (req, res, next) => {
    ...
    if (!notification) return next(sendError(message404, 404));
